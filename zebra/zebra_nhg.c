...
/*id counter for pic_nexthop */
uint32_t id_counter_pic = 1000;
...
bool zebra_nhg_hash_equal(const void *arg1, const void *arg2)
{
	...
	if (nhe1->afi != nhe2->afi)
		return false;
    /*pic nhe different from normal nhe*/
	if (CHECK_FLAG(nhe1->flags, NEXTHOP_GROUP_PIC_NHT) != CHECK_FLAG(nhe2->flags, NEXTHOP_GROUP_PIC_NHT))
		return false;
				...
				struct nexthop *nexthop, afi_t afi, int type, bool pic, bool color_only)
{
	...
	for (nh = nexthop; nh; nh = nh->next) {
		...
		if (IS_ZEBRA_DEBUG_NHG_DETAIL)
			zlog_debug("%s: head %p nh %pNHv sidlist_name %s discriminator %u type %d flags 0x%x",
...
struct zebra_sr_policy *zebra_sr_policy_match_by_nexthop(struct nexthop *nexthop)
{
			   ...
			   afi_t afi, bool from_dplane, bool pic)
{
	...
	if (!pic && !CHECK_FLAG(lookup->flags, NEXTHOP_GROUP_BSID)) {
		lookup_tmp = zebra_nhe_copy_no_recurse(lookup);
		free_flag = true;
	} else
		lookup_tmp = lookup;

	if (!CHECK_FLAG(lookup->flags, NEXTHOP_GROUP_BSID)) {
		if (lookup_tmp->id)
			   ...
			   bool from_dplane, bool pic, bool color_only)
{
	...
	if (IS_ZEBRA_DEBUG_NHG_DETAIL)
		zlog_debug("%s: id %u, nhg %p, vrf %d, type %d, depends %p, nexthop type %d, pic %s",
			   __func__, id, nhg, vrf_id, type,
			   nhg_depends, nexthop->type, (pic ? "true":"false"));
...
void zebra_nhg_free(struct nhg_hash_entry *nhe)
{
	if (IS_ZEBRA_DEBUG_NHG_DETAIL) {
		...
		if (nhe->nhg.nexthop && nhe->nhg.nexthop->next)
			zlog_debug("%s: nhe %p (%pNG), refcnt %d, segment_ref %d",
				   __func__, nhe, nhe, nhe->refcnt, nhe->refcnt);
		else
			zlog_debug("%s: nhe %p (%pNG), refcnt %d, segment_ref %d, NH %pNHv",
				   __func__, nhe, nhe, nhe->refcnt, nhe->segment_ref,
				   nhe->nhg.nexthop);
	}
...
void zebra_nhg_seg_increment_ref(struct nhg_hash_entry *nhe)
{
	...
	/*if (!zebra_nhg_segdepends_is_empty(nhe))
		nhg_segment_tree_increment_ref(&nhe->nhg_segdepends);*/
}
					    ...
					    struct zebra_sr_policy *policy)
{
	...
	if (nexthop->nh_srv6) {
		...
		if (CHECK_FLAG(newhop->flags, NEXTHOP_FLAG_SRV6_TUNNEL))
...
static void nexthop_seg_set_resolved(struct nexthop *nexthop,
					    struct zebra_sr_policy *policy,
						uint32_t policy_num)
{
	struct nexthop *resolved_hop;
	...
	if (CHECK_FLAG(nexthop->flags, NEXTHOP_FLAG_SRV6_BSID))
		SET_FLAG(resolved_hop->flags, NEXTHOP_FLAG_SRV6_BSID);
		...
		if (policy && policy->status == ZEBRA_SR_POLICY_UP) {
			...
			for (path_num = 0; path_num < policy->srv6_segment_list.path_num; path_num++) {
				...
				if (CHECK_FLAG(policy->srv6_segment_list.sidlists[path_num].flags, SRV6_SID_LIST_HIDDEN)
					&& CHECK_FLAG(nexthop->flags, NEXTHOP_FLAG_SRV6_BSID))
					continue;
				nexthop_seg_set_resolved(nexthop, policy, path_num);
				resolved = 1;
			}
				     ...
				     struct nhg_hash_entry *nhe)
{
	...
	if (IS_ZEBRA_DEBUG_NHG_DETAIL)
		zlog_debug("%s: re %p, nexthop %pNHv, type %d", __func__,
			re, nexthop, nexthop->type);
	...
	case NEXTHOP_TYPE_IPV4_SEGMENTLIST:
		...
		if (nexthop_seg_active(nexthop, nhe))
			SET_FLAG(nexthop->flags, NEXTHOP_FLAG_ACTIVE);
		else {
			UNSET_FLAG(nexthop->flags, NEXTHOP_FLAG_ACTIVE);
...
#if 0
	/* Install origin nhe for srv6 case*/
	nh = nhe->nhg.nexthop;
