			   ...
			   bool from_dplane, bool pic)
{
	...
	if (IS_ZEBRA_DEBUG_NHG_DETAIL)
		zlog_debug("%s: id %u, nhg %p, vrf %d, type %d, depends %p, nexthop_type %d, pic %s",
			   __func__, id, nhg, vrf_id, type,
			   nhg_depends, nexthop_type, (pic ? "true":"false"));
						     ...
						     bool from_dplane, bool pic)
{
	...
	if (IS_ZEBRA_DEBUG_NHG_DETAIL)
		zlog_debug("%s: nh %pNHv => %p (%u) flags 0x%x",
			   __func__, nh, nhe, nhe ? nhe->id : 0, nh->flags);
...
zebra_nhg_rib_find_nhe(struct nhg_hash_entry *rt_nhe, afi_t rt_afi)
{
	...
	if (IS_ZEBRA_DEBUG_NHG_DETAIL)
		zlog_debug("%s: rt_nhe %p (%u), flags 0x%x",
			__func__, rt_nhe, rt_nhe->id, rt_nhe->flags);
...
void zebra_nhg_free(struct nhg_hash_entry *nhe)
{
	if (IS_ZEBRA_DEBUG_NHG_DETAIL) {
		...
		if (nhe->nhg.nexthop && nhe->nhg.nexthop->next)
			zlog_debug("%s: nhe %p (%u), refcnt %d, segment_ref %d",
				   __func__, nhe, nhe->id, nhe->refcnt, nhe->refcnt);
		else
			zlog_debug("%s: nhe %p (%u), refcnt %d, segment_ref %d, NH %pNHv",
				   __func__, nhe, nhe->id, nhe->refcnt, nhe->segment_ref,
				   nhe->nhg.nexthop);
	}
...
int nexthop_active_update(struct route_node *rn, struct route_entry *re)
{
	...
	if (IS_ZEBRA_DEBUG_NHG_DETAIL)
		zlog_debug("%s: re %p nhe %p (%u), curr_nhe %p, flags %d, org flags %d",
			   __func__, re, re->nhe, re->nhe->id,
			   curr_nhe, curr_nhe->flags, re->nhe->flags);
