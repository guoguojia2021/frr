...
TEST_F(ZebraNhgsExample, SinglePolicySingleRoute)
{
	...
	for (const auto &route : routes) {
		struct zapi_route api_route =
			RibUtils::get_zapi_route_from_t(route);
...
TEST_F(ZebraNhgsExample, MultiPolicyHybridRouteAddRoute)
{
	...
	zebra_vrf *zvrf = policy_utils.get_mock_zvrf();
	zserv *client = policy_utils.get_mock_client();
	for (const auto &route : test_case.initial_state.rib) {
		struct zapi_route api_route =
			RibUtils::get_route_multi_policies(route);
		struct stream *s = RibUtils::fill_stream_with_route(&api_route);
		EXPECT_NE(s, nullptr);
	...
	struct zapi_route api_route =
		RibUtils::get_route_multi_policies(test_case.input_route);
	struct stream *s = RibUtils::fill_stream_with_route(&api_route);
	EXPECT_NE(s, nullptr);
