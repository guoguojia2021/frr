...
static void parse_json_route_nexthops_t(json_object *obj,
					api_route_nexthop_t *res)
{
	prefix_t prefix;
	json_object_object_foreach(obj, key, val)
	{
...
static void parse_json_input_routeadd(json_object *obj, api_route_t *res)
{
	json_object_object_foreach(obj, key, val)
	{
		if (strcmp(key, "RoutePrefix") == 0) {
			...
			for (int i = 0; i < nexthop_num; i++) {
				res->api_nexthops.push_back({});
				parse_json_route_nexthops_t(
					json_object_array_get_idx(val, i),
					&res->api_nexthops.back());
			}
		} else {
			std::cerr
...
static void print_nexthop(const struct nexthop_t &n, int level)
{
	...
	cout << " " << string((level + 1) * 2, '-') << " nexthop"
	     ...
	     << " sidlist_name: " << n.sidlist_name << endl;
...
// Route prefixes are IPv4
void print_api_route(const struct api_route_t &route)
{
	char ipv4[INET_ADDRSTRLEN];
	inet_ntop(AF_INET, route.prefix.endpoint, ipv4, sizeof(ipv4));

	std::cout << "api_route " << ipv4 << "/" << route.prefix.prefixlen;

	for (const auto &nh : route.api_nexthops) {
		char gate_str[INET6_ADDRSTRLEN];
		inet_ntop(AF_INET6, nh.gate, gate_str, sizeof(gate_str));
		std::cout << " -- gate " << gate_str << " color " << nh.color;
	}
	std::cout << std::endl;
}
