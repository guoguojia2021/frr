			   ...
			   bool show_defaults)
{
	...
	if (IS_SET_IPv4_NH(action)) {
	...
	} else if (IS_SET_AS_OVERWRITE(action)) {
		vty_out(vty, " set as-path overwrite %s\n",
			yang_dnode_get_string(
				dnode,
		...
		vty_out(vty, " set rmac %s\n",
			yang_dnode_get_string(
				dnode,
				"./rmap-set-action/frr-bgp-route-map:rmac"));
	} else if (IS_SET_VNI(action)) {
		vty_out(vty, " set vni %s\n",
			yang_dnode_get_string(
				dnode,
